...
#   bash app_map.sh ./sim                       → mapea ./sim → SIM_CONTEXT.md
#   bash app_map.sh --audit                     → mapea + audita nav (ruta default)
#   bash app_map.sh --audit ./nav/ ./sim        → mapea sim + audita ./nav/
...
PYTREE
  ...
  if [[ -n "$URLS_F" ]]; then
    ns=$(grep -m1 "app_name\s*=" "$URLS_F" 2>/dev/null | grep -oP "'[^']+'" | tr -d "'" || echo "")
...
section "▸ Uso rápido"
echo ""
echo "  bash scripts/app_map.sh                      # mapa de ./analyst"
echo "  bash scripts/app_map.sh ./sim                # mapa de ./sim"
echo "  bash scripts/app_map.sh --audit              # mapa + auditoría nav"
echo "  bash scripts/app_map.sh --no-map --audit     # solo auditoría"
echo ""
ok "¡Listo!"
