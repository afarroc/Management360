# Servicios Complementarios de Management360

Este directorio contiene servicios independientes que corren junto a la aplicación Django principal.

## 📡 Media Server

Servidor HTTP para servir archivos multimedia con soporte CORS, optimizado para Termux/Android.

### Uso básico:
```bash
cd services
python media_server.py --host 0.0.0.0 --port 8001
```

### Opciones:
- `--host`: IP del servidor (default: 192.168.18.51)
- `--port`: Puerto (default: 8000, recomiendo 8001 para evitar conflicto con Django)
- `--directory`: Directorio a servir (default: ./media)
- `--log-level`: DEBUG, INFO, WARNING, ERROR

### Notas importantes:
- El servidor incluye soporte CORS para acceso desde cualquier origen
- Permite subida de archivos vía POST multipart/form-data
- Diseñado específicamente para entorno Termux/Android

## 📚 Documentación Adicional del Media Server
