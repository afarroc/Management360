...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      # Positional directory → app mode (no mode keyword needed)
      if [[ -d "$1" ]]; then
        APP_ARG="$1"
        [[ -z "$MODE" ]] && MODE="app"
      fi
...
_local_apps() {
  # Apps = subdirs that have __init__.py AND (models.py OR apps.py)
  find "$PROJECT_ROOT" -maxdepth 2 \
    ...
    | while read -r d; do
        ...
        # Skip root itself and single-level hidden/static dirs
        [[ "$rel" == "" || "$rel" == "." ]] && continue
        [[ "$rel" == *"/"* ]] && continue  # skip nested
...
# =============================================================================
# MODE: urls — replicate core/url_map_view in bash
# =============================================================================
_mode_urls() {
...
# =============================================================================
# MODE: app — deep analysis of a single app (same as old app_map.sh)
# =============================================================================
_mode_app() {
  local APP_DIR
...
PYEOF
  ...
  echo "---"
  echo ""
  echo "## Funciones por app (vistas publicas)"
  echo ""
  echo "> Solo funciones \`def\` publicas de \`views.py\` y \`views/*.py\`"
  echo ""

  for app in "${APPS[@]}"; do
    local app_dir="$PROJECT_ROOT/$app"
    declare -a vfiles=()
    [[ -f "$app_dir/views.py" ]] && vfiles+=("$app_dir/views.py")
    for vf in "$app_dir/views/"*.py; do [[ -f "$vf" ]] && vfiles+=("$vf"); done
    [[ ${#vfiles[@]} -eq 0 ]] && continue

    local app_fns=""
    for vf in "${vfiles[@]}"; do
      local rel="${vf#$PROJECT_ROOT/}"
      local fns; fns=$(grep -n "^def [a-z]" "$vf" 2>/dev/null | grep -v "^[0-9]*:def _" \
                       | sed "s|^\\([0-9]*\\):\\(.*\\)|  L\\1  \\2  ($rel)|" || true)
      [[ -n "$fns" ]] && app_fns+="$fns"$'\n'
    done
    [[ -z "$app_fns" ]] && continue
...
_mode_project() {
  ...
  local FULL_OUT="$PROJECT_ROOT/$OUT"
  ...
  for app in "${APPS[@]}"; do
    ...
    APP_MODELS[$app]=0
    if [[ -f "$models_f" ]]; then
      APP_MODELS[$app]=$(grep -c "^class " "$models_f" 2>/dev/null || true)
      APP_MODELS[$app]="${APP_MODELS[$app]:-0}"
      APP_MODELS[$app]=$(echo "${APP_MODELS[$app]}" | tr -d '[:space:]')
    fi

    # URLs count + namespace
    local urls_f="$app_dir/urls.py"
    APP_URLS[$app]=0
    APP_NS[$app]=""
    if [[ -f "$urls_f" ]]; then
      local raw_urls; raw_urls=$(grep -c "path(" "$urls_f" 2>/dev/null || true)
...
HEADER
  ...
  for app in "${APPS[@]}"; do
    local note=""
    [[ "$app" == "analyst"   ]] && note="Plataforma de datos (5 fases)"
    ...
    [[ "$app" == "courses"   ]] && note="Cursos, leciones, curriculum"
  ...
  echo ""
  echo "\`\`\`"
  # Only show top-level app dirs to avoid hanging on large projects
...
PYTREE1
  ...
  echo ""
  echo "Para acceder al mapa de URLs en tiempo real:"
  echo ""
  echo "\`\`\`"
...
bash m360_map.sh app ./analyst

# Auditoría de residuos en nav:
bash m360_map.sh app ./analyst --audit

# Mapa de URLs en consola:
bash m360_map.sh urls
...
# =============================================================================
# MODE: audit — nav HTML residue check (from app_map.sh)
# =============================================================================
_mode_audit() {
  local APP_DIR=""
...
echo ""
section "▸ Uso rápido"
echo "  bash m360_map.sh                        # todo el proyecto"
echo "  bash m360_map.sh app ./analyst          # app profunda"
echo "  bash m360_map.sh app ./analyst --audit  # app + auditoría nav"
echo "  bash m360_map.sh urls                   # mapa de URLs"
echo ""
ok "¡Listo!"
